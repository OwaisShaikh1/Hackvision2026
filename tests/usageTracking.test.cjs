...
describe("Usage Tracking Logic", () => {
  ...
  describe("usage-based priority decisions", () => {
    const { isHighPriority } = require("../src/priority.cjs");
...
describe("URL Pattern Matching", () => {
  ...
  test("partial URL match works for significance", () => {
    ...
    expect(
      isHighPriority(null, "https://example.com/api/v1/users/123", config),
    ).toBe(true);

    // Should match /analytics pattern
    expect(
      isHighPriority(
        { url: "/x", count: 1000, lastAccessed: Date.now() },
  ...
  test("non-matching URLs fall through to dynamic logic", () => {
    ...
    expect(
      isHighPriority(lowUsage, "https://example.com/normal/path", config),
    ).toBe(false);
  });
});

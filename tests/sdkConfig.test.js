...
describe('SDK Configuration', () => {
  ...
  describe('priority tuning options', () => {
    ...
    test('networkQuality accepts valid options', () => {
      expect(buildConfig({ networkQuality: 'auto' }).networkQuality).toBe('auto');
      expect(buildConfig({ networkQuality: 'fast' }).networkQuality).toBe('fast');
      expect(buildConfig({ networkQuality: 'slow' }).networkQuality).toBe('slow');
    });
...
describe('URL Pattern Matching Simulation', () => {
  ...
  test('external API matching', () => {
    ...
    expect(urlMatches('https://jsonplaceholder.typicode.com/posts', apis)).toBe(true);
